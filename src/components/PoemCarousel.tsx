...
interface PoemCarouselProps {
  ...
  handleImageError: (poemId: string) => void
              ...
              {(poem.coverSmallImage || poem.coverImage) && !imageErrors[poem.id] ? (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center items-center p-3 text-center">
                    <h3 className="text-white text-sm font-bold leading-tight line-clamp-2 mb-1">{poem.title}</h3>
                    <div className="w-8 h-px bg-white opacity-50 mb-1"></div>
                    <p className="text-white text-xs opacity-75 truncate w-full">{poem.poetName}</p>
                  </div>
